...
== What is `{image-name}`?
The `{image-name}` image ships with git and my own `.gitconfig` file.

CAUTION: Since this image contains my own `.gitconfig` file this image is not universally usable.
